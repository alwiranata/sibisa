...
const SensorAngin = () => {
	...
	const data = {
		...
		datasets: [
			{
				label: "Kecepatan Angin (mm/s)",
				data: limitedData.map((item) => item.kecepatan_angin),
				fill: false,
	...
	const options: ChartOptions<"line"> = {
		...
		scales: {
			...
			y: {title: {display: true, text: "Kecepatan Angin (mm/s)"}},
		},
	}
		...
		<div className='flex flex-col'>
			...
			<header className='flex flex-row justify-around mt-[50px] mb-[101px]'>
				<div className='flex flex-col justify-center w-[239px] gap-2'>
					<h1 className='text-[#2F4752] font-bold'>Kecepatan Angin</h1>
				</div>
				<img
					className='w-[50px]'
			...
			<div className='flex flex-row justify-around'>
				<div className='flex flex-col gap-[26px]'>
					...
					{/* Kecepatan */}
					<div className='flex flex-col items-center justify-center bg-[#2F4752] rounded-2xl w-[137px] h-[100px]'>
						<h1 className='font-bold text-white'>Kecepatan</h1>
						<p className='m-2 w-[90%] h-full bg-white text-[#2F4752] rounded-[8px] text-center flex justify-center items-center'>
							{latestData ? `${latestData.kecepatan_angin} mm/s` : "—"}
						</p>
					</div>
				</div>
				...
				<div className='flex flex-col items-center justify-center rounded-2xl h-[352px] w-[175px] bg-[#2F4752]'>
					...
					<p
						className={`m-2 w-[90%] h-full rounded-[8px] bg-white text-center flex justify-center items-center font-black  ${
							latestData?.status_angin === "Angin Kencang"
								? "text-red-600"
								: latestData?.status_angin === "Angin Sedang"
								? "text-yellow-500"
								: "text-green-500"
								
						}`}
					>
						{latestData ? latestData.status_angin : "—"}
					</p>
				</div>
			</div>
		</div>
