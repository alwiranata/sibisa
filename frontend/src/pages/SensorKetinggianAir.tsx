...
const SensorKetinggianAir = () => {
	...
	// Dataset ketinggian air
	const data = {
		labels,
		datasets: [
			{
				label: "Ketinggian Sensor Dari Air (cm)",
	...
	const options: ChartOptions<"line"> = {
		...
		scales: {
			...
			y: {title: {display: true, text: "Ketinggian Sensor Dari Air (cm)"}},
		},
	}
		...
		<div className='flex flex-col'>
			...
			<header className='flex flex-row justify-around mt-[50px] mb-[101px]'>
				<div className='flex flex-col justify-center w-[239px] gap-2'>
					<h1 className='text-[#2F4752] font-bold'>Ketinggian Air</h1>
				</div>
				<img
					className='w-[50px]'
			...
			<div className='flex flex-row justify-around'>
				<div className='flex flex-col gap-[26px]'>
					...
					{/* Ketinggian */}
					<div className='flex flex-col items-center justify-center bg-[#2F4752] rounded-2xl w-[137px] h-[100px]'>
						<h1 className='font-bold text-white'>Ketinggian</h1>
						<p className='m-2 w-[90%] h-full bg-white text-[#2F4752] rounded-[8px] text-center flex justify-center items-center'>
							{latestData ? `${latestData.ketinggian_air} cm` : "—"}
						</p>
					</div>
				</div>
				...
				<div className='flex flex-col items-center justify-center rounded-2xl h-[352px] w-[175px] bg-[#2F4752]'>
					...
					<p
						className={`m-2 w-[90%] h-full bg-white  rounded-[8px] text-center flex justify-center items-center font-black ${
							latestData?.status_air === "Bahaya"
								? "text-red-600"
								: latestData?.status_air === "Siaga"
								? "text-yellow-500"
								: "text-green-600"
						}`}
					>
						{latestData ? latestData.status_air : "—"}
					</p>
				</div>
			</div>
		</div>
...
export default SensorKetinggianAir
