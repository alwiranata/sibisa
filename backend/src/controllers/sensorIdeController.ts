...
export const addSensorDataIde = async (req: Request, res: Response) => {
	try {
		...
		if (dataSensor.curah_hujan > 0 && dataSensor.curah_hujan <= 3) {
			status_hujan = "Cerah"
		} else if (dataSensor.curah_hujan > 3 && dataSensor.curah_hujan < 15) {
		...
		let status_angin = ""
		if (dataSensor.kecepatan_angin > 2 && dataSensor.kecepatan_angin <= 11) {
			status_angin = "Angin Normal"
		} else if (dataSensor.kecepatan_angin > 11) {
